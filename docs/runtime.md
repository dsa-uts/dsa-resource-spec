...
[課題定義](resource.md) を実行するシステムの契約。この Go モジュールは定義の検証・素材の読み込み・ZIP 生成を担当する。以下の実行、採点、権限制御、提出物の正規化は Backend・Judge の責務とする。
...
stdout / stderr は UTF-8 テキストとして扱う。デコードできない stream は比較失敗とする。

| `match` | 比較方法 |
| --- | --- |
| `exact` | 正規化せず完全一致。末尾の改行も比較する。 |
| `easy` | 以下の正規化後、行数・各行の要素数・要素の順序と内容が一致すること。 |
| `sorted` | `easy` の正規化後、各行の要素を文字列の辞書順で昇順ソートして比較する。行順は変えない。 |
