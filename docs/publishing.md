...
このリポジトリで課題定義を検証し、サンドボックスイメージを GHCR へ、解決済み課題 JSON を main の `release/` へ公開する。GitHub Release は使わない。実データとして `ex1/` と `ex2/`、ビルド定義として `sandbox/` と `sandbox-runner/` を登録している。`ex2/` は private ジョブ・専用テストケースと提出物の例を除いて取り込んでいる。旧リポジトリの公開履歴・イメージの固定情報は移行しない。
